    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 border-b border-gray-200 flex items-center justify-between gap-3 flex-wrap">
                <div>
                    <h2 class="text-xl font-semibold text-gray-900">จัดการสถานที่สอบ</h2>
                    <p class="mt-1 text-sm text-gray-500">Create / Read / Update / Delete สถานที่สอบ</p>
                </div>
                <a href="{{ route('staff.test-locations.create') }}" wire:navigate
                   class="inline-flex items-center px-4 py-2 bg-primary-600 border border-transparent rounded-md text-xs font-semibold text-white uppercase tracking-widest hover:bg-primary-700">
                    เพิ่มสถานที่ใหม่
                </a>
            </div>
            ...
            <div class="p-6 border-b border-gray-100 bg-gray-50 grid grid-cols-1 md:grid-cols-3 gap-3">
                <div class="md:col-span-2">
                    ...
                    <input type="text" wire:model.live.debounce.300ms="search"
                           ...
                           placeholder="ค้นหาจากชื่อสถานที่หรือรหัสสถานที่">
                </div>
                <div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ชื่อสถานที่</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ที่อยู่</th>
                            <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">จำนวนที่รับได้</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        @forelse ($this->locations as $location)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{{ $location->code }}</td>
                                <td class="px-6 py-4 text-sm text-gray-900">{{ $location->name }}</td>
                                <td class="px-6 py-4 text-sm text-gray-700">{{ $location->address ?: '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-900">{{ number_format($location->capacity) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    @if ($location->is_active)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">เปิดใช้งาน</span>
                                    @else
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700">ปิดใช้งาน</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="inline-flex items-center gap-2">
                                        <a href="{{ route('staff.test-locations.edit', $location->id) }}" wire:navigate
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-10 text-center text-sm text-gray-500">ไม่พบข้อมูลสถานที่สอบ</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/40">
                <div class="w-full max-w-md rounded-xl bg-white shadow-xl border border-gray-200">
                    <div class="p-5 border-b border-gray-100">
                        <h3 class="text-base font-semibold text-gray-900">ยืนยันการลบสถานที่สอบ</h3>
                    </div>
                    <div class="p-5">
                        ...
                        <p class="mt-2 text-xs text-red-600">ถ้ามีผู้สมัครใช้งานอยู่ ระบบจะไม่อนุญาตให้ลบ</p>
                    </div>
